...
export function useVideoPrefetch(srcs: string[]): PrefetchState {
  ...
  useEffect(() => {
    ...
    Promise.all(
      ...
      if (cancelled) {
        // Clean up if component unmounted before fetch completed
    ...
    return () => {
      cancelled = true;
      // Revoke only on full unmount, not on re-renders
      objectUrls.forEach(URL.revokeObjectURL);
    };
