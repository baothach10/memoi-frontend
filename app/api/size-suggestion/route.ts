...
const sizeSuggestionSchema = jsonSchema<{
  ...
  properties: {
    suggestedSize: {
      type: 'string',
      description: 'The name of the suggested size (e.g., SERENE, MUSE, LUMINOUS)',
    },
    reasoning: {
      type: 'string',
      description: 'Brief explanation for the suggestion based on body type and clothing preference (max 2 sentences)',
    },
  },
  required: ['suggestedSize', 'reasoning'],
  additionalProperties: false,
});

export async function POST(req: Request) {
  try {
    const { height, weight, age, chest, waist, hip, preference, sizingRanges } = await req.json();
...
Your task is to recommend the best clothing size for a customer based on their measurements and preferences.
...
5. Provide a brief, warm, and professional reasoning (max 2 sentences).`,
    });

    return NextResponse.json(object);
  } catch (error) {
    console.error('Size suggestion error:', error);
    return NextResponse.json({ error: 'Failed to generate size suggestion' }, { status: 500 });
  }
}
