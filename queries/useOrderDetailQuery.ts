...
function transformRawOrderDetails(raw: RawOrderDetails): OrderDetails {
  ...
  return {
    ...
    shipTo: raw.destination || `${bi.city}, ${bi.country}`,
    status: raw.status,
    ...
    tier_discount_amount: raw.tier_discount_amount,
    promo_discount_amount: raw.promo_discount_amount,
    shipping_fee: raw.shipping_fee,
    progress: [
      { label: "Order Placed", date: new Date(raw.created_at).toLocaleDateString("en-GB", { day: 'numeric', month: 'short' }), completed: true, active: raw.status === 'IN_PROGRESS' },
      { label: "Processing", date: "", completed: raw.status === 'COMPLETED', active: false },
      { label: "Shipped", date: "", completed: raw.status === 'COMPLETED', active: false },
      { label: "Delivered", date: "", completed: raw.status === 'COMPLETED', active: false },
    ],
  };
}
